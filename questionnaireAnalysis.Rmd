...
df <- df %>%
  mutate(across(all_of(columns_to_convert), as.numeric), show_col_types = FALSE)
...
## 1.4. Kandidattest Usage Review (Past and Future)
```{r}

# Past Usage?
##unique(df$char_past_candidate_test)

## "yes"
count_kandTestPast_yes <- matrixStats::count(df$char_past_candidate_test == "yes")
cat(
  "No. of participants who have used some sort of Kandidattest to inform themselves about Danish Election choices in the past:",
  count_kandTestPast_yes,
  "\n\n"
)

## "no"
count_kandTestPast_no <- matrixStats::count(df$char_past_candidate_test == "no")
cat(
  "No. of participants who have NOT used some sort of Kandidattest to inform themselves about Danish Election choices in the past:",
  count_kandTestPast_no,
  "\n\n"
)
...
# Future Usage?
##unique(df$char_future_candidate_test)

## "yes"
count_kandTestFuture_yes <- matrixStats::count(df$char_future_candidate_test == "yes")
cat(
  "No. of participants who do plan to use some sort of Kandidattest to inform themselves about Danish Election choices:",
  count_kandTestFuture_yes,
  "\n\n"
)

## "no"
count_kandTestFuture_no <- matrixStats::count(df$char_future_candidate_test == "no")
cat(
  "No. of participants who do NOT plan on using any Kandidattest to inform themselves about Danish Election choices:",
  count_kandTestFuture_no,
  "\n\n"
)
...
## Wilcoxon Signed-Rank t-Tests comparing pre-tool and post-tool usage !

stat_wResults <- list() # empty list to store the results
...
for (pair in groupPairs) {
  ...
  # perfomring the paired t-test!
  stat_wResults <- wilcox.test(group1, group2, paired = TRUE,
                          alternative = "two.sided")
  
  # storing the result in the pre-established list!
  test_name <- paste("Wilcoxon Signed-Rank t-test between", pair[1], "and", pair[2])
  stat_wResults[[test_name]] <- stat_wResults
  ...
  # performing the hypothesis test using results!
  if (!is.null(stat_wResults$p.value[1]) && stat_wResults$p.value[1] > 0.05) {
    cat("Fail to reject null hypothesis that there is no significant difference between the groups\n\n")
  } else if (!is.null(stat_tResults$p.value[1]) && stat_tResults$p.value[1] <= 0.05) {
    cat("Reject the null hypothesis that there is no significant difference between the groups\n\n")
  } else {
...
* okay... these aren't looking great :,) -- hmmmmmm I'm not sure how to deal with this,, maybe I'll check for outliers to see if there's anything we can salvage buuuuut yeahh..
* I'll also see if I can get some more people to try it out..
